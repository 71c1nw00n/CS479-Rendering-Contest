...
if [ "$FLASHATTN" = true ] ; then
    if [ "$PLATFORM" = "cuda" ] ; then
        FLASHATTN_CUDA_HOME=${CUDA_HOME}
        if [ -n "$CONDA_PREFIX" ] && [ -x "$CONDA_PREFIX/bin/nvcc" ] ; then
            FLASHATTN_CUDA_HOME=$CONDA_PREFIX
        fi
        if [ "$PYTORCH_VERSION" = "2.4.0" ] && [ "$CUDA_VERSION" = "11.8" ] ; then
            ...
            pip install psutil
            CUDA_HOME=$FLASHATTN_CUDA_HOME pip install flash-attn --no-build-isolation
        fi
    elif [ "$PLATFORM" = "hip" ] ; then
...
if [ "$NVDIFFRAST" = true ] ; then
    if [ "$PLATFORM" = "cuda" ] ; then
        ...
        NVDIFFRAST_CUDA_HOME=${CUDA_HOME}
        if [ -n "$CONDA_PREFIX" ] && [ -x "$CONDA_PREFIX/bin/nvcc" ] ; then
            NVDIFFRAST_CUDA_HOME=$CONDA_PREFIX
        fi
        if command -v gcc-11 >/dev/null 2>&1 && command -v g++-11 >/dev/null 2>&1 ; then
            CUDA_HOME=$NVDIFFRAST_CUDA_HOME CC=gcc-11 CXX=g++-11 pip install --no-build-isolation /tmp/extensions/nvdiffrast
        else
            CUDA_HOME=$NVDIFFRAST_CUDA_HOME pip install --no-build-isolation /tmp/extensions/nvdiffrast
        fi
    else
...
if [ "$DIFFOCTREERAST" = true ] ; then
    if [ "$PLATFORM" = "cuda" ] ; then
        mkdir -p /tmp/extensions
        if [ ! -d /tmp/extensions/diffoctreerast/.git ] ; then
            git clone --recurse-submodules https://github.com/JeffreyXiang/diffoctreerast.git /tmp/extensions/diffoctreerast
        fi
        DIFFOCTREERAST_CUDA_HOME=${CUDA_HOME}
        if [ -n "$CONDA_PREFIX" ] && [ -x "$CONDA_PREFIX/bin/nvcc" ] ; then
            DIFFOCTREERAST_CUDA_HOME=$CONDA_PREFIX
        fi
        if command -v gcc-11 >/dev/null 2>&1 && command -v g++-11 >/dev/null 2>&1 ; then
            CUDA_HOME=$DIFFOCTREERAST_CUDA_HOME CC=gcc-11 CXX=g++-11 pip install --no-build-isolation /tmp/extensions/diffoctreerast
        else
            CUDA_HOME=$DIFFOCTREERAST_CUDA_HOME pip install --no-build-isolation /tmp/extensions/diffoctreerast
        fi
    else
...
if [ "$MIPGAUSSIAN" = true ] ; then
    if [ "$PLATFORM" = "cuda" ] ; then
        mkdir -p /tmp/extensions
        if [ ! -d /tmp/extensions/mip-splatting/.git ] ; then
            git clone https://github.com/autonomousvision/mip-splatting.git /tmp/extensions/mip-splatting
        fi
        MIPGAUSSIAN_CUDA_HOME=${CUDA_HOME}
        if [ -n "$CONDA_PREFIX" ] && [ -x "$CONDA_PREFIX/bin/nvcc" ] ; then
            MIPGAUSSIAN_CUDA_HOME=$CONDA_PREFIX
        fi
        if command -v gcc-11 >/dev/null 2>&1 && command -v g++-11 >/dev/null 2>&1 ; then
            CUDA_HOME=$MIPGAUSSIAN_CUDA_HOME CC=gcc-11 CXX=g++-11 pip install --no-build-isolation /tmp/extensions/mip-splatting/submodules/diff-gaussian-rasterization/
        else
            CUDA_HOME=$MIPGAUSSIAN_CUDA_HOME pip install --no-build-isolation /tmp/extensions/mip-splatting/submodules/diff-gaussian-rasterization/
        fi
    else
